(()=>{
  ...
  const labGuide=path.match(/\/weeks\/13-lab-learning-path\/lab\d{2}\.html$/i);
  const resource=path.match(/\/resources\/([^/]+)\/(?:index\.html)?$/i);
  if(practice){
  ...
  else if(resource&&file!=='resources'){back={href:`${BASE}resources/`,label:'Back to Resources'};}
