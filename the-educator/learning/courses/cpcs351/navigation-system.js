(()=>{
  ...
  function togglePanel(btn){const p=panel(),open=p.hidden;p.hidden=!open;$$('[data-nav-panel]').forEach(x=>x.setAttribute('aria-expanded','false'));if(open){btn.setAttribute('aria-expanded','true');renderMap();setTimeout(()=>$('input',p)?.focus(),0)}else btn.focus()}
  ...
  function a11y(){$('#skip-link')?.remove();const main=$('main');if(main){main.id=main.id||'main-study-content';main.tabIndex=-1}document.addEventListener('keydown',e=>{if(e.key==='Escape')closePanel()});document.addEventListener('click',e=>{const p=$('.edu-nav-panel');if(p&&!p.hidden&&!e.target.closest('.edu-nav-panel')&&!e.target.closest('[data-nav-panel]'))closePanel()})}
