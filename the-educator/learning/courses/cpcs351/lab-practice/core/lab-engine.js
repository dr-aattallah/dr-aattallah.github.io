...
export class LabEngine {
  ...
  restoreState() {
    ...
    this.stageIndex = Number.isInteger(saved.stageIndex)
      ? Math.max(0, Math.min(saved.stageIndex, this.activeStageIds.length))
      : 0;

    // If a student refreshed after checking a stage but before pressing Next,
    // resume after that completed stage instead of reopening it for a second answer.
    while (
      this.stageIndex < this.activeStageIds.length &&
      this.results.has(this.activeStageIds[this.stageIndex])
    ) {
      this.stageIndex += 1;
    }
  ...
  saveState() {
    this.state.save({
      ...
      activeStageIds: this.activeStageIds
    });
  }
  ...
  renderClassification(host, stage) {
    ...
    host.querySelector('[data-check]').addEventListener('click', () => {
      ...
      this.finishStage(stage, correct, stage.feedback || {
        why: 'Review the categories used in this activity.',
        consequence: 'Correct classification supports engineering reasoning.'
      });
    });
  }
  ...
  renderSequence(host, stage) {
    ...
    host.querySelector('[data-check]').addEventListener('click', () => {
      ...
      this.finishStage(stage, correct, stage.feedback || {
        why: correct ? 'You reconstructed the lifecycle in the intended Topic 01 order.' : `The Topic 01 sequence is: ${stage.answer.join(' → ')}.`,
        consequence: 'Lifecycle order helps you reason about how decisions and evidence flow from concept through operation and change.'
      });
    });
  }
  ...
  renderMatching(host, stage) {
    ...
    host.querySelector('[data-check]').addEventListener('click', () => {
      ...
      this.finishStage(stage, correct, stage.feedback || {
        ...
        consequence: 'Reason from concept to engineering action, not labels in isolation.'
      });
    });
  }
  ...
  finishStage(stage, correct, feedback) {
    ...
    this.root.querySelector('[data-interaction]').querySelectorAll('button, select, input').forEach(control => {
  ...
  renderSummary() {
    const orderedResults = this.mission.stages
      .map(stage => [stage.id, this.results.get(stage.id)])
      .filter(([, result]) => result);

    // A stale or incomplete saved state should never produce a misleading debrief.
    ...
    const readiness = correct === this.mission.stages.length
      ? 'Mission Ready'
      : correct >= 6
        ? 'Nearly Ready'
        : correct >= 4
