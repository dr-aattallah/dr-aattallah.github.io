(()=>{
  const pages=[
    ['index.html','Smart Parcel Locker — Anchor Scenario'],
    ['library-example.html','Use Case View — Who Wants What?'],
    ['class-diagram.html','Class View — What Exists and Relates?'],
    ['state-revision.html','Sequence View — Who Interacts, and When?'],
    ['activity-revision.html','State View — What Changes Over Time?'],
    ['atm-example.html','Activity View — How Does Work Flow?'],
    ['same-scenario.html','Same Scenario, Five Models'],
    ['consistency.html','Consistency Across Models'],
    ['integrated-case.html','Integrated Case Study'],
    ['model-selection.html','Model Selection Challenges'],
    ['studio.html','Integrated Modeling Studio'],
    ['revision-check.html','Integrated Review & Exam Readiness']
  ];
  const currentFile=location.pathname.split('/').pop()||'index.html';
  const current=Math.max(0,pages.findIndex(p=>p[0]===currentFile));
